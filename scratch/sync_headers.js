...
// Extract nav HTML from contact.html
const navHtmlMatch = contactHtml.match(/<!-- =+[\r\n\s]+HEADER \/ NAVIGATION \(Synchronized eMMSOFT Navbar\)[\s\S]*?<\/nav>/);
if (!navHtmlMatch) {
  console.error('Failed to extract nav HTML from contact.html');
  process.exit(1);
}
const standardNavHtml = navHtmlMatch[0];

// Standardize nav CSS (including .mobile-panel a styling)
...
// Target files
const targets = ['index.html', 'about.html', 'team.html', 'ceo-message.html', 'cto-message.html'];

targets.forEach(file => {
  let content = fs.readFileSync(file, 'utf8');

  // Replace Nav HTML
  // Match either with or without preceding comment
  const navHtmlRegex = /(<!-- =+[\r\n\s]+HEADER[\s\S]*?<\/nav>|<nav class="nav"[\s\S]*?<\/nav>)/;
  if (!navHtmlRegex.test(content)) {
    console.error(`Could not find nav HTML in ${file}`);
    return;
  }
  content = content.replace(navHtmlRegex, standardNavHtml);

  // Replace Nav CSS
  // Identify the CSS block starting with .nav { or /* NAVBAR... */ down to the mobile query
  const navCssRegex = /(\/\* =+[\r\n\s]+NAVBAR[\s\S]*?@media \(max-width: 900px\) \{[\s\S]*?\.nav-burger \{ display: flex; \}\s*\}|\.nav\s*\{[\s\S]*?@media \(max-width: 900px\) \{[\s\S]*?\.nav-burger \{ display: flex; \}\s*\})/;
  if (navCssRegex.test(content)) {
    content = content.replace(navCssRegex, standardNavCss);
  } else {
    console.warn(`Nav CSS regex did not match in ${file}, checking manual replacement`);
  }

  // Ensure JS includes burger toggle and sticky scroll
  // Check if siteNav scroll and burgerBtn are present
  if (!content.includes('burgerBtn.addEventListener')) {
    console.warn(`Burger listener missing in ${file}`);
  }
