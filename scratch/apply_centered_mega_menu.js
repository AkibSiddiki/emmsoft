...
files.forEach(f => {
  ...
  // Replace standard navbar CSS block
  const cssRegex = /\/\* =+[\r\n\s]+NAVBAR \(Synchronized System\)[\s\S]*?\.nav-burger \{ display: flex; \}\s*\}/;
  if (cssRegex.test(content)) {
    content = content.replace(cssRegex, standardNavCssCentered.trim());
    fs.writeFileSync(f, content, 'utf8');
    console.log(`Successfully updated centered navbar CSS in ${f}`);
  } else {
    console.error(`Failed to match standard navbar CSS in ${f}`);
  }
});

console.log('Centering update complete.');
