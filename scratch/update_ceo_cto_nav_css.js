...
// Update ceo-message.html
let ceo = fs.readFileSync('ceo-message.html', 'utf8');
const ceoRegex = /\/\*\s*Navbar\s*\*\/[\s\S]*?(?=\/\* =+[\r\n\s]+CEO MESSAGE HERO)/i;
if (ceoRegex.test(ceo)) {
  ceo = ceo.replace(ceoRegex, standardNavCss + '\n\n    ');
  fs.writeFileSync('ceo-message.html', ceo, 'utf8');
  console.log('Successfully updated ceo-message.html navbar CSS');
} else {
  console.error('ceoRegex did not match');
}

// Update cto-message.html
let cto = fs.readFileSync('cto-message.html', 'utf8');
const ctoRegex = /\/\*\s*Navbar\s*\*\/[\s\S]*?(?=\/\* =+[\r\n\s]+CTO MESSAGE HERO)/i;
if (ctoRegex.test(cto)) {
  cto = cto.replace(ctoRegex, standardNavCss + '\n\n    ');
  fs.writeFileSync('cto-message.html', cto, 'utf8');
  console.log('Successfully updated cto-message.html navbar CSS');
} else {
  console.error('ctoRegex did not match');
}
